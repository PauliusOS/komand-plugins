...
- For runtime validation, hand off to `$ios-debugger-agent` to build + launch
  and surface the app inside Komand's embedded iPhone preview (the iOS tab
  next to Browser), then trigger the intent via Shortcuts/Siri/Spotlight to
  verify the handoff lands in the right place. Do not open Simulator.app.
