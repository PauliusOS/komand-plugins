...
When the audit includes runtime validation, hand off to `$ios-debugger-agent`
to build + launch and surface the app inside Komand's embedded iPhone preview
(the iOS tab next to Browser) — scrolling and animation behavior is easiest
to judge live. Do not open Simulator.app.
