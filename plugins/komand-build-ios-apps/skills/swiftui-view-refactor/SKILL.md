...
Refactors must not regress behavior. After any non-trivial view split, hand
off to `$ios-debugger-agent` to build + launch and surface the app inside
Komand's embedded iPhone preview (the iOS tab next to Browser) to eyeball
the refactored screen. Do not open Simulator.app.
