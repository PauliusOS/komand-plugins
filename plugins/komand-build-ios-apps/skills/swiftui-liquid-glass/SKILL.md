...
Liquid Glass is visual-heavy — validate each change by running the app and
looking at it. After you ship a change, hand off to `$ios-debugger-agent` to
build + launch and surface the app inside Komand's embedded iPhone preview
(the iOS tab next to Browser). Do not open Simulator.app.
